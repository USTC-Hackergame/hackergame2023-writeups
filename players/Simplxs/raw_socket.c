...
void create_syn_packet(struct sockaddr_in *src, struct sockaddr_in *dst, char **out_packet, int *out_packet_len)
{
	...
	iph->version = 4;
	iph->tos = 0;
	iph->tot_len = sizeof(struct iphdr) + sizeof(struct tcphdr) + 10;
	...
	psh.tcp_length = htons(sizeof(struct tcphdr) + 10);
	int psize = sizeof(struct pseudo_header) + sizeof(struct tcphdr) + 10;
	// fill pseudo packet
	char *pseudogram = malloc(psize);
	memcpy(pseudogram, (char *)&psh, sizeof(struct pseudo_header));
	memcpy(pseudogram + sizeof(struct pseudo_header), tcph, sizeof(struct tcphdr));
	memcpy(pseudogram + sizeof(struct pseudo_header) + sizeof(struct tcphdr), "GET / HTTP", 10);

	memcpy(datagram + 40, "GET / HTTP", 10);
...
void create_data_packet(struct sockaddr_in *src, struct sockaddr_in *dst, int32_t seq, int32_t ack_seq, char *data, int data_len, char **out_packet, int *out_packet_len)
{
	...
	// set payload
	char *payload = datagram + sizeof(struct iphdr) + OPT_SIZE + sizeof(struct tcphdr);
	...
	memcpy(pseudogram + sizeof(struct pseudo_header), tcph, sizeof(struct tcphdr));
	memcpy(pseudogram + sizeof(struct pseudo_header) + sizeof(struct tcphdr), "GET / HTTP", 10);
	memcpy(pseudogram + sizeof(struct pseudo_header) + sizeof(struct tcphdr) + OPT_SIZE, data, data_len);
...
int main()
{
	...
	// // bind socket to source IP address
