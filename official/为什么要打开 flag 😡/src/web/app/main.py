...
def index():
    if request.method == "POST":
        ...
        if stage not in (1, 2):
            return render_template('index.html', result="选择的小题错误。")
        print(f"get upload from {get_user_id()}, {sha256(file)}")
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.connect((os.environ[f"nc_host{stage}"], int(os.environ["nc_port"])))
        ...
        if buf == b"Base64 of binary: ":
            ...
            while True:
                ...
                if not data:
                    break
                buf += data
        return render_template('index.html', result=buf.decode(), selected=stage)
    else:
